...
from DialogEvaluator_emory import DialogEvaluator
...
if __name__ == '__main__':
    ...
    # train_data = TorchWrappedDataset(instances, tokenizer_roberta, max_seq_length=25)
    # train_batch_size = 2
    ...
    # valid_data = TorchWrappedDataset(valid_instances, tokenizer_roberta, max_seq_length=25)
    ...
    #  You can even specify devicepad='cuda:xxx' here since only the tokenizer is
    #  called in the TorchWrappedDataset and there are no Tensors in the tokenizer
    # daily dialogue max_seq_length=128, emory: 108
    tokenizer_bert = ROBERTA(tokenizer_name, max_seq_length=108, devicepad='cpu')

    logging.info('Read test dataset')
    # daily dialogue max_seq_length = 36, emory: 25
    test_data = TorchWrappedDataset(instances, tokenizer_bert, max_seq_length=36)
    # can be the same the train_batch_size
    train_batch_size = 2

    # If loading it's the saving configuration of model_dlg's bert that determines
    #  it's device
    model_save_path = ('../save/saved-model-emory')
    ...
    model_dlg.evaluate(evaluator, output_path='../save/saved-model-emory')
    ...
    # a_dialogue_flattened = model_bert.tokenizer.batch_decode(first_batch[0].view(-1, 108), skip_special_tokens=True, clean_up_tokenization_spaces=True)
