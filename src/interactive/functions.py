...
def load_data(dataset, opt):
    ...
    encoder_path = "datasets/comet_saved_lm_model/encoder_bpe_40000.json"
    bpe_path = "datasets/comet_saved_lm_model/vocab_40000.bpe"
...
def load_atomic_data(opt):
    ...
    if opt.data.get("maxe1", None) is None:
        ...
        opt.data.maxe2 = 35
        opt.data.maxr = 1
    path = "datasets/kb_data/atomic/processed/generation/{}.pickle".format(
...
def load_conceptnet_data(opt):
    ...
    if opt.data.get("maxr", None) is None:
        if opt.data.rel == "language":
            ...
            opt.data.maxr = 1
    path = "datasets/kb_data/conceptnet/processed/generation/{}.pickle".format(
    utils.make_name_string(opt.data))
...
def set_atomic_inputs(input_event, category, data_loader, text_encoder):
    XMB = torch.zeros(1, data_loader.max_event + 256).long().to(cfg.device)
