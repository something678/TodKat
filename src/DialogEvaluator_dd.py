...
class DialogEvaluator:
    ...
    def __call__(
        ...
        for step, batch in enumerate(tqdm(self.dataloader, desc="Evaluating")):
            ...
            pred_list.extend(torch.argmax(var_uttrs, dim=1).tolist())
            grd_list.extend(var_labels.tolist())
        ...
        logging.info("Accuracy (daily dialogue returns macro_f1): {:.4f} ({}/{})\n".format(
            accuracy, correct, total))
        # print(torch.argmax(var_uttrs, dim=1).tolist())
        ...
        macro_f1 = metrics.f1_score(
            ...
            labels=[1, 2, 3, 4, 5, 6],
            average='weighted')
        logging.info("F1-micro except neutral: {:.4f} ({}/{})\n".format(macro_f1, correct, total))
        micro_f1 = metrics.f1_score(
            pred_list,
            grd_list,
            labels=[1, 2, 3, 4, 5, 6],
            average='micro')
        logging.info("F1-macro except neutral: {:.4f} ({}/{})\n".format(micro_f1, correct, total))
