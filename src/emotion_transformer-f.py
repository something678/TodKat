        ...
        for epoch in trange(epochs, desc="Epoch"):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05):
                ...
                for train_idx in range(num_train_objectives):
                    ...
                    # self._model = torchScholar( and self._model(
                    # in scholar.py, to obtain a loss, which has .backward()
                    # currently no another RNN added
...
if __name__ == '__main__':
    ...
    # 'bert-base-uncased'
    tokenizer_bert = BERT(tokenizer_name, max_seq_length=108, devicepad='cuda:1')
    # devicepad is only used for the evaluation of the model.

    logging.info('Read train dataset')
    train_data = TorchWrappedDataset(instances, tokenizer_bert)
    train_batch_size = 2
    ...
    model_tfe = TransformerUnit(
        d_model=tokenizer_bert.get_word_embedding_dimension(),
        n_heads=8,
    ...
    valid_data = TorchWrappedDataset(valid_instances, tokenizer_bert)
    ...
    #  device
    tokenizer_bert = BERT(tokenizer_name, devicepad='cuda:1')
